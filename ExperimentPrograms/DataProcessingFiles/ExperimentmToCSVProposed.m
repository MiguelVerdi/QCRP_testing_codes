...
TUpdateCon = data.T_update_con; %Array with the time of iteration.


%disp(clusters)
%Transforms the TUpdateCon to a number of iteration array, so where the GPCA is is known 
index = 1;
for i = 1:length(TUpdateCon) - 1   
    GPCA = [GPCA,index]; 
    index = index + 1 + TUpdateCon(i); %Gets the Iteration variable. 
end %end of i loop;
...
for I = 1:simulationIterations  %Iterates over all the time segments, it uses the variable I so the time segments are not just of 1 sec gap.
    ...
    isGPCA = ismember(I, GPCA); %Maps the segments are GPCA (since the format is diferent)
